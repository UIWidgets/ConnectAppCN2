...
namespace ConnectApp.Common.Util {
    public class EmojiResourceConfiguration {
        ...
        public EmojiResourceConfiguration(
            ...
            for (var i = 0; i < emojiCodes.Count; i++) {
                this.emojiLookupTable[emojiCodes[index: i]] = i;
            }
    ...
    public class EmojiUtils {
        ...
        public static Dictionary<int, int> emojiLookupTable {
            get { return configuration.emojiLookupTable; }
        }
        ...
        public static Rect getUVRect(int code) {
            var exist = emojiLookupTable.TryGetValue(key: code, out var index);
            if (!exist) {
                exist = emojiLookupTable.TryGetValue(key: emptyEmojiCode, value: out index);
                D.assert(() => {
        ...
        public static bool isSingleCharEmoji(int c) {
            return isSingleCharNonEmptyEmoji(c: c) || isEmptyEmoji(c: c);
        }

        public static bool isSingleCharNonEmptyEmoji(int c) {
            return c <= 0xffff && emojiLookupTable.ContainsKey(key: c);
        }

        public static bool isEmptyEmoji(int c) {
            return c == emptyEmojiCode && !emojiLookupTable.ContainsKey(key: c);
        }
        ...
        public static List<string> splitByEmoji(string text) {
            ...
            for (var i = 0; i < text.Length; i++) {
                if (i < text.Length - 1 && char.IsHighSurrogate(text[index: i]) && char.IsLowSurrogate(text[i + 1])) {
                    if (currentEmoji != true) {
                        if (i > start) {
                            list.Add(text.Substring(startIndex: start, i - start));
                            start = i;
                        }
                    }
                ...
                else if (isSingleCharEmoji(text[index: i])) {
                    if (currentEmoji != true) {
                        if (i > start) {
                            list.Add(text.Substring(startIndex: start, i - start));
                            start = i;
            ...
            if (start < text.Length) {
                list.Add(text.Substring(startIndex: start));
            }
