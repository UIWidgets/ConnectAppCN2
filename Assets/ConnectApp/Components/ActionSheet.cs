...
namespace ConnectApp.Components {
    ...
    public class ActionSheet : StatelessWidget {
            ...
            return new Container(
                ...
                child: new Column(
                    ...
                    children: new List<Widget> {
                        ...
                        _buildButtons(items: this.items, context: context),
        ...
        static Widget _buildButtons(List<ActionSheetItem> items, BuildContext context) {
            if (items.isNullOrEmpty()) {
                return new Container();
            }

            var widgets = new List<Widget>();
            var normalWidgets = new List<Widget>();
            var destructiveWidgets = new List<Widget>();
            var cancelWidgets = new List<Widget>();
            items.ForEach(item => {
                Color titleColor;
                switch (item.type) {
                ...
                Widget widget = new GestureDetector(
                    onTap: () => {
                        ActionSheetUtils.hiddenModalPopup(context: context);
                        item.onTap?.Invoke();
                    },
                    child: new Container(
                        alignment: Alignment.center,
    ...
    public static class ActionSheetUtils {
        public static void showModalActionSheet(
            ...
            Navigator.push<object>(context: context, route: route).then(_ => onPop?.Invoke());
        }

        public static void hiddenModalPopup(BuildContext context) {
            // TODO: Finn can pop
                    ...
                    children: new List<Widget> {
                        Positioned.fill(
                            new Opacity(
                                ...
                                child: this.overlayBuilder(context: context)
                            )
                        ),
                        result
                    });
