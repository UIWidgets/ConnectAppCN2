...
namespace ConnectApp.Components {
    ...
    public class _KingKongViewState : State<KingKongView> {
        public override void initState() {
            base.initState();
            var currentTabIndex = StoreProvider.store.getState().tabBarState.currentTabIndex;
            if (currentTabIndex != 0) {
                return;
            }

            // TODO: Finn hidden tips
            // Future.delayed(TimeSpan.FromMilliseconds(1)).then(_ => {
                                        ...
                                        children: new List<Widget> {
                                            ...
                                            Positioned.fill(
                                                new Container(
                                                    ...
                                                    child: Image.file($"image/{imageName}.png")
                                                )
                                            )
                                        }
                                    )
    ...
    public class RecommendBlogger : StatelessWidget {
        ...
        void _onFollow(UserType userType, string userId, BuildContext context) {
            if (this.isLoggedIn) {
        ...
        public override Widget build(BuildContext context) {
            ...
            if (this.bloggerIds.Count <= 3) {
                for (var i = 0; i < this.bloggerIds.Count; i++) {
                    var bloggerId = this.bloggerIds[index: i];
                    ...
                    if (this.userDict.ContainsKey(key: rankData.itemId)) {
                        ...
                        if (i == this.bloggerIds.Count - 1) {
                            children.Add(new Container(
                                ...
                                child: this._buildBlogger(user: user, context: context)
                            ));
                        }
                        else {
                            children.Add(this._buildBlogger(user: user, context: context));
                        }
                    }
                }
            }
                ...
                for (var index = 0; index < _showBloggerCount; index++) {
            ...
            return new Container(
                ...
                child: new Stack(
                    ...
                    children: new List<Widget> {
                        Image.file(
                            "image/recommend-blogger-bg.png",
                            fit: BoxFit.fill
                        ),
                        new Column(
                            children: new List<Widget> {
        ...
        Widget _buildBlogger(User user, BuildContext context) {
                    ...
                    child: new ClipRRect(
                        ...
                        child: new Column(
                            children: new List<Widget> {
                                    ...
                                    child: new Stack(
                                        ...
                                        children: new List<Widget> {
                                            user.coverImage.isNotEmpty()
                                                ? Image.network(src: user.coverImage, fit: BoxFit.fill)
                                                : Image.file(
                                                    "image/blogger-avatar-pattern.png",
                                                    fit: BoxFit.fill
                                                ),
                                            Positioned.fill(
                                                new Center(
                                ...
                                new Padding(
                                    padding: EdgeInsets.only(16, 16, 16, 6),
                                ...
                                new Expanded(
                                    child: new Column(
                                        ...
                                        children: new List<Widget> {
                                            new Flexible(
                                                child: CImageUtils.GenBadgeImage(
                                                    badges: user.badges,
                                                    null,
                                ...
                                new Padding(
                                    ...
                                    child: new FollowButton(
                                        userType: userType,
                                        () => this._onFollow(userType: userType, userId: user.id, context: context)
                                    )
                                )
                            }
                        )
                ...
                child: new Container(
                    ...
                    child: new ClipRRect(
                        ...
                        child: new Stack(
                            ...
                            children: new List<Widget> {
                                Image.file(
                                    "image/blogger-more-pattern.png",
                                    fit: BoxFit.fill
                                ),
                                Positioned.fill(
                                    new Column(
                                        mainAxisAlignment: MainAxisAlignment.center,
                                        children: this._buildAvatars(bloggerIds: bloggerIds)
                                    )
                                )
                            }
                        )
        ...
        List<Widget> _buildAvatars(List<string> bloggerIds) {
            ...
            bloggerIds.ForEach(bloggerId => {
                var index = bloggerIds.IndexOf(item: bloggerId);
                var rankData = this.rankDict.ContainsKey(key: bloggerId)
                    ? this.rankDict[key: bloggerId]
                    : new RankData();
                if (this.userDict.ContainsKey(key: rankData.itemId)) {
                    ...
                    items.Add(new Positioned(
                            child: Avatar.User(user: user, 44, true),
                            left: left
                        )
                    );
                }
