...
namespace ConnectApp.Components {
    ...
    public static class CustomDismissibleUtil {
        ...
        public static readonly Curve ResizeTimeCurve = new Interval(0.4f, 1.0f, curve: Curves.ease);
    ...
    public abstract class SlideToDismissDelegate {
        ...
        public Widget buildActions(BuildContext context, CustomDismissibleDelegateContext ctx,
            ...
            if (ctx.state.overallMoveAnimation.value > ctx.state.totalActionsExtent) {
                return this.buildActionsWhileDismissing(context: context, ctx: ctx);
            }

            return dismissibleDelegate.buildActions(context: context, ctx: ctx);
        }
    ...
    public class SlideToDismissDrawerDelegate : SlideToDismissDelegate {
        public SlideToDismissDrawerDelegate(
            ...
            SlideActionWillBeDismissed onWillDismiss = null,
            bool closeOnCanceled = false
        ) : base(dismissThresholds: dismissThresholds,
            onResize: onResize,
        ...
        protected override Widget buildActionsWhileDismissing(BuildContext context,
            ...
            var animation = new OffsetTween(
                begin: Offset.zero,
                ctx.createOffset(value: ctx.state.dragSign)
            ).animate(parent: ctx.state.overallMoveAnimation);
                    ...
                    children: new List<Widget> {
                        Positioned.fill(
                            new LayoutBuilder(builder: (_context, constraints) => {
                                    ...
                                    var actionExtent =
                                        ctx.getMaxExtent(constraints: constraints) * ctx.state.widget.actionExtentRatio;
                                    var totalExtent = ctx.getMaxExtent(constraints: constraints);
                                    ...
                                    for (var index = 0; index < count; index++) {
                                        var extentAnimation = new FloatTween(
                                            ...
                                            new CurvedAnimation(
                                                ...
                                                new Interval(begin: ctx.state.totalActionsExtent, 1)
                                            )
                                        );
                                        extentAnimations.Add(item: extentAnimation);
                                    }
                                            ...
                                            for (var index = 0; index < ctx.state.actionCount; index++) {
                                                ...
                                                var widget = ctx.createPositioned(
                                                    position: (float) actionExtent *
                                                              (ctx.state.actionCount - index - 1),
                                                    extent: extentAnimations[index: index].value,
                                                    child: ctx.state.actionDelegate.build(
                                                        context: context,
                                                        index: displayIndex,
                                                        animation: ctx.state.overallMoveAnimation,
                                                        mode: ctx.state.renderingMode
                                                    )
                                                );
    ...
    public class SlideActionBuilderDelegate : SlideActionDelegate {
        ...
        public override Widget build(BuildContext context, int index, Animation<float> animation,
            CustomDismissibleMode mode) {
            return this.builder(context: context, index: index, animation: animation, mode: mode);
        }
    }
    ...
    public class SlideActionListDelegate : SlideActionDelegate {
        ...
        public override Widget build(BuildContext context, int index, Animation<float> animation,
            CustomDismissibleMode mode) {
            return this.actions[index: index];
        }
    }
    ...
    public class CustomDismissibleDelegateContext {
        ...
        public List<Widget> buildActions(BuildContext context) {
            ...
            for (var index = 0; index < this.state.actionCount; index++) {
                var widget = this.state.actionDelegate.build(context: context, index: index,
                    animation: this.state.actionsMoveAnimation,
                    mode: CustomDismissibleMode.slide);
                widgets.Add(item: widget);
            }
        ...
        public Offset createOffset(float value) {
            return this.state.directionIsXAxis
                ? new Offset(dx: value, 0)
                : new Offset(0, dy: value);
        }
    ...
    public abstract class CustomDismissibleStackDelegate : CustomDismissibleDelegate {
        protected CustomDismissibleStackDelegate(
            float? fastThreshold = null
        ) : base(fastThreshold: fastThreshold) {
        }
        ...
        public override Widget buildActions(BuildContext context, CustomDismissibleDelegateContext ctx) {
            ...
            ).animate(parent: ctx.state.actionsMoveAnimation);

            if (ctx.state.actionsMoveAnimation.value != 0.0f) {
                return new Container(
                    child: new Stack(
                        children: new List<Widget> {
                            this.buildStackActions(context: context, ctx: ctx),
                            new SlideTransition(
                                position: animation,
                                child: ctx.state.widget.child
                            )
            ...
            return Positioned.fill(
                new LayoutBuilder(builder: (_context, constraints) => {
                    return new AnimatedBuilder(
                        ...
                        (cxt, child) => {
                            var widgets = new List<Widget>();
                            ctx.buildActions(context: cxt).ForEach(item => {
                            ...
                            return new Stack(
                                children: new List<Widget> {
                                    ctx.createPositioned(
                                        position: 0,
                                        extent:
                                        ctx.getMaxExtent(constraints: constraints) * animation.value.abs(),
                                        child: new Flex(
    ...
    public class CustomDismissibleBehindDelegate : CustomDismissibleStackDelegate {
        ...
        protected override Widget buildStackActions(BuildContext context, CustomDismissibleDelegateContext ctx) {
            return Positioned.fill(
                new LayoutBuilder(builder: (_context, constraints) => {
                    var widgets = new List<Widget>();
                    ctx.buildActions(context: _context).ForEach(item => {
                        var widget = new Expanded(child: item);
                        widgets.Add(item: widget);
                    });
                    return new Stack(
                        children: new List<Widget> {
                            ctx.createPositioned(
                                position: 0,
                                extent:
                                ctx.getMaxExtent(constraints: constraints) * ctx.state.totalActionsExtent,
                                child: new Flex(
                                    direction: ctx.state.widget.direction,
                                    children: widgets
                                )
            ...
            return Positioned.fill(
                new LayoutBuilder(builder: (_context, constraints) => {
                    var totalExtent =
                        ctx.getMaxExtent(constraints: constraints) * ctx.state.totalActionsExtent;
                    var animation =
                        new FloatTween(begin: -totalExtent, 0).animate(parent: ctx.state.actionsMoveAnimation);
                    return new AnimatedBuilder(
                        animation: ctx.state.actionsMoveAnimation,
                        (cxt, child) => {
                            var widgets = new List<Widget>();
        ...
        protected override Widget buildStackActions(BuildContext context, CustomDismissibleDelegateContext ctx) {
            return Positioned.fill(
                new LayoutBuilder(builder: (_context, constraints) => {
                    ...
                    var actionExtent = ctx.getMaxExtent(constraints: constraints) * state.widget.actionExtentRatio;
                    ...
                    for (var index = 0; index < count; index++) {
                        ...
                        ).animate(parent: actionsMoveAnimation);
                    ...
                    return new AnimatedBuilder(
                        ...
                        (cxt, child) => {
                            ...
                            for (var index = 0; index < count; index++) {
                                ...
                                var widget = ctx.createPositioned(
                                    position: animations[index: index].value,
    ...
    public class CustomDismissibleController {
        ...
        public void setActiveState(CustomDismissibleState newValue) {
            ...
            if (this.onSlideAnimationChanged != null) {
                this._slideAnimation?.removeListener(listener: this._handleSlideIsOpenChanged);
                if (this.onSlideIsOpenChanged != null) {
                    this._slideAnimation = newValue?.overallMoveAnimation;
                    this._slideAnimation?.addListener(listener: this._handleSlideIsOpenChanged);
                    if (this._slideAnimation == null) {
                        this._isSlideOpen = false;
                        this.onSlideIsOpenChanged(value: this._isSlideOpen);
                    }
                }

                this.onSlideAnimationChanged(value: newValue?.overallMoveAnimation);
            }
        }
    ...
    public class CustomDismissible : StatefulWidget {
        ...
        public static CustomDismissible builder(
            ...
            return new CustomDismissible(
                key: key,
                child: child,
                dismissibleDelegate: dismissibleDelegate,
    ...
    public class CustomDismissibleState : AutomaticKeepAliveClientWithTickerProviderStateMixin<CustomDismissible>,
        ...
        public float dismissThreshold {
            get {
                return this.widget.slideToDismissDelegate.dismissThresholds[key: this.actionType] ??
        ...
        public override void initState() {
            ...
            this._overallMoveController.addStatusListener(listener: this._handleDismissStatusChanged);
            this._actionsMoveController = new AnimationController(duration: this.widget.movementDuration, vsync: this);
            this._actionsMoveController.addStatusListener(listener: this._handleShowAllActionsStatusChanged);
            this._dragExtent = 0;
        }
        ...
        void _addScrollingNotifierListener() {
            if (this.widget.closeOnScroll) {
                this._scrollPosition = Scrollable.of(context: this.context)?.position;
                if (this._scrollPosition != null) {
                    this._scrollPosition.isScrollingNotifier.addListener(listener: this._isScrollingListener);
                }
            }
        }
        ...
        void _removeScrollingNotifierListener() {
            if (this._scrollPosition != null) {
                this._scrollPosition.isScrollingNotifier.removeListener(listener: this._isScrollingListener);
            }
        }

        public void open() {
            this._actionsMoveController.fling();
            this._overallMoveController.animateTo(target: this.totalActionsExtent,
                curve: Curves.easeIn,
            ...
            if (this.dismissible) {
                if (status == AnimationStatus.completed &&
                    ...
                    if (this.widget.slideToDismissDelegate.onWillDismiss == null ||
                        this.widget.slideToDismissDelegate.onWillDismiss(actionType: this.actionType)) {
        ...
        void _handleDismiss() {
            ...
            if (slideToDismissDelegate.onDismissed != null) {
                slideToDismissDelegate.onDismissed(actionType: this.actionType);
            }
        }

        void _startResizeAnimation() {
            D.assert(this._overallMoveController != null);
            D.assert(result: this._overallMoveController.isCompleted);
            ...
            if (slideToDismissDelegate.resizeDuration == null) {
                ...
                this._resizeController.addListener(listener: this._handleResizeProgressChanged);
                ...
                this.setState(() => {
                    ...
                    this._resizeAnimation = new FloatTween(1, 0).animate(
                        new CurvedAnimation(parent: this._resizeController,
                            curve: CustomDismissibleUtil.ResizeTimeCurve));
                });
            }
        }
        ...
        public override Widget build(BuildContext context) {
            base.build(context: context); // See AutomaticKeepAliveClientMixin.
            ...
            if (this.actionType == CustomDismissibleActionType.primary && this.widget.actionDelegate != null &&
                ...
                if (this.dismissible) {
                    content = this.widget.slideToDismissDelegate.buildActions(
                        context: context,
                        new CustomDismissibleDelegateContext(this), dismissibleDelegate: this.widget.dismissibleDelegate
                    );
