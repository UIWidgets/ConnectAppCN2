    ...
    public class _CustomInputState : State<CustomInput> {
                ...
                if (selection.start > 0) {
                    this._textController.value = new TextEditingValue(
                        ...
                        TextSelection.collapsed(selection.start -
                                                _codeUnitLengthAt(value: this._textController.value)));
                }
            }
            else {
                this._textController.value = new TextEditingValue(
        ...
        void _onSubmitted(string text) {
            if (string.IsNullOrWhiteSpace(value: text)) {
                CustomDialogUtils.showToast(context: this.context, "不能发送空消息", iconData: CIcons.error_outline);
                return;
            }
            ...
            return new CustomTextField(
                ...
                onPressEmoji: () => {
                    if (this.showEmojiBoard) {
                        ...
                        Future.delayed(TimeSpan.FromMilliseconds(200)).then(
                            _ => {
                                if (this.mounted) {
                        ...
                        Future.delayed(TimeSpan.FromMilliseconds(100))
                            .then(_ => TextInputPlugin.TextInputHide());
                    }
                }
            );
        }
