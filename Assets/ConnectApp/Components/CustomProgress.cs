...
namespace ConnectApp.Components {
    ...
    class _CustomProgressState : SingleTickerProviderStateMixin<CustomProgress> {
        ...
        Widget _buildIndicator(float animationValue) {
            return new Container(
                ...
                child: new CustomPaint(
                    painter: new _CustomProgressPainter(backgroundColor: this.widget.backgroundColor,
                        this.widget.valueColor?.value ?? CColors.PrimaryBlue, value: this.widget.value,
        ...
        public override Widget build(BuildContext context) {
            if (this.widget.value != null) {
                return this._buildIndicator(animationValue: this._controller.value);
            }

            return new AnimatedBuilder(
                animation: this._controller.view,
                (cxt, child) => this._buildIndicator(animationValue: this._controller.value)
            );
        }
    }
    ...
    class _CustomProgressPainter : AbstractCustomPainter {
        ...
        public override void paint(Canvas canvas, Size size) {
            ...
            canvas.drawRect(Offset.zero & size, paint: paint);
            ...
            void drawBar(float x, float width) {
                ...
                canvas.drawRect(new Offset(dx: x, 0.0f) & new Size(width: width, height: size.height), paint: paint);
            }
            ...
            if (this.value != null) {
                ...
                drawBar(x: x1, width: width1);
                drawBar(x: x2, width: width2);
            }
        }
