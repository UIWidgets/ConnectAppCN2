...
namespace ConnectApp.Components {
    ...
    class _EmojiBoardState : State<EmojiBoard>, TickerProvider {
                        ...
                        new GestureDetector(
                            ...
                            child: new Container(
                                ...
                                child: new Center(
                                    child: new Text(
                                        "发送",
                                        style: CTextStyle.PMediumBlue.defaultHeight()
                                    )
                                )
                            )
                        )
        ...
        List<Widget> _buildEmojiBoardPages(BuildContext context) {
            var emojiButtonSize = _getEmojiButtonSize(context: context);
            var emojiPages = new List<Widget>();
            for (var i = 0; i < emojiList.Count; i += emojiBoardPageSize) {
                var rows = new List<Widget>();
                for (var j = 0; j < emojiBoardColumnSize; j++) {
                    var emojis = new List<Widget>();
                    for (var k = 0; k < emojiBoardRowSize; k++) {
        ...
        Widget _buildDeleteButton(BuildContext context) {
            ...
            return new GestureDetector(
                ...
                child: new Container(
                    ...
                    child: new Center(
                        child: new Icon(
                            icon: CIcons.outline_delete_keyboard,
        ...
        Widget _buildEmojiButton(BuildContext context, int i, int j, int k) {
            var emojiButtonSize = _getEmojiButtonSize(context: context);
            var index = i + j * emojiBoardRowSize + k;
