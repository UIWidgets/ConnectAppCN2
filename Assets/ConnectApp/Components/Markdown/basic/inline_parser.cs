...
namespace ConnectApp.Components.Markdown.basic {
    public class InlineParser {
        ...
        public InlineParser(string source, Document document) {
            ...
            this.syntaxes.AddRange(collection: document.inlineSyntaxes);

            var documentHasCustomInlineSyntaxes = document.inlineSyntaxes
                .Any((s) => !document.extensionSet.inlineSyntaxes.Contains(item: s));
            ...
            this.syntaxes.AddRange(collection: _defaultSyntaxes);

            if (this.document.encodeHtml) {
                this.syntaxes.AddRange(collection: _htmlSyntaxes);
            }
        ...
        internal List<Node> parse() {
            ...
            while (!this.isDone) {
                ...
                foreach (var state in this._stack.reversed()) {
                    if (state.syntax != null && state.tryMatch(this)) {
                        shouldContinue = true;
                        break;
                    }
                }
                ...
                foreach (var syntax in this.syntaxes) {
        ...
        internal int charAt(int index) {
            return this.source.codeUnitAt(i: index);
        }


        internal void writeText() {
            this.writeTextRange(start: this.start, end: this.pos);
            this.start = this.pos;
        }
        ...
        internal void writeTextRange(int start, int end) {
            ...
            var text = this.source.substring(start: start, end: end);
    ...
    public abstract class InlineSyntax {
        ...
        protected InlineSyntax(string pattern, int startCharacter = -1) {
            this.pattern = new Regex(pattern: pattern, options: RegexOptions.Multiline);
        ...
        internal virtual bool tryMatch(InlineParser parser, int startMatchPos = -1) {
            ...
            if (this._startCharacter != -1 && parser.source.codeUnitAt(i: startMatchPos) != this._startCharacter) {
                return false;
            }

            var startMatch = this.pattern.matchAsPrefix(content: parser.source, start: startMatchPos);
            ...
            if (this.onMatch(parser: parser, match: startMatch)) {
                parser.consume(length: startMatch.Groups[0].Length);
            }
    ...
    class TextSyntax : InlineSyntax {
        ...
        internal TextSyntax(string pattern, string sub = "") : base(pattern: pattern) {
            this.substitute = sub;
        }


        internal override bool onMatch(InlineParser parser, Match match) {
            if (string.IsNullOrEmpty(value: this.substitute)) {
                // Just use the original matched text.
                parser.advanceBy(length: match.Groups[0].Length);
                return false;
            }

            // Insert the substitution.
            parser.addNode(new Text(text: this.substitute));
            return true;
        }
    }
...
// Insert the substitution.
    ...
    class EmailAutolinkSyntax : InlineSyntax {
        ...
        internal override bool onMatch(InlineParser parser, Match match) {
            ...
            var anchor = Element.text("a", Utils.escapeHtml(html: url));
            anchor.attributes["href"] = ("mailto:" + url); // TODO: Uri.encodeFull
            parser.addNode(node: anchor);
            return true;
        }
    }
...
// underscores may be present in the last two segments of the domain.
        ...
        static string domain = string.Format("[{0}][{0}.]+", arg0: domainPart);
...
// be considered part of the autolink
        ...
        static Regex regExpTrailingPunc =
            new Regex(string.Format("{0}*{1}", arg0: truncatingPunctuationPositive, "$"));
        ...
        internal override bool tryMatch(InlineParser parser, int startMatchPos = 0) {
            return base.tryMatch(parser: parser, parser.pos > 0 ? parser.pos - 1 : 0);
        }
        ...
        internal override bool onMatch(InlineParser parser, Match match) {
            ...
            if (url[0] == '>' || url.startsWith(pattern: regExpWhiteSpace)) {
...
// https://github.github.com/gfm/#example-600
            if (url.endsWith(')')) {
                var opening = this._countChars(input: url, '(');
                var closing = this._countChars(input: url, ')');
                if (closing > opening) {
...
// https://github.github.com/gfm/#example-599
            var trailingPunc = regExpTrailingPunc.Match(input: url);
...
// https://github.github.com/gfm/#example-602
            if (url.endsWith(';')) {
                var entityRef = regExpEndsWithColon.Match(input: url);
...
// The scheme http will be inserted automatically
            ...
            parser.addNode(node: anchor);
            parser.consume(length: matchLength);
            return false;
        }
    ...
    class _DelimiterRun {
        ...
        internal static _DelimiterRun tryParse(InlineParser parser, int runStart, int runEnd) {
            ...
            if (runStart == 0) {
                ...
                preceding = parser.source.substring(runStart - 1, end: runStart);
            }

            precededByPunctuation = punctuation.Contains(value: preceding);
            ...
            followedByPunctuation = punctuation.Contains(value: following);

// http://spec.commonmark.org/0.28/#left-flanking-delimiter-run
            if (whitespace.Contains(value: following)) {
                leftFlanking = false;
            }
            else {
                leftFlanking = !followedByPunctuation ||
                               whitespace.Contains(value: preceding) ||
                               precededByPunctuation;
            }

// http://spec.commonmark.org/0.28/#right-flanking-delimiter-run
            if (whitespace.Contains(value: preceding)) {
                rightFlanking = false;
            }
            else {
                rightFlanking = !precededByPunctuation ||
                                whitespace.Contains(value: following) ||
                                followedByPunctuation;
            }
...
// Could not parse a delimiter run.
    ...
    class TagSyntax : InlineSyntax {
        ...
        internal TagSyntax(string pattern, bool requiresDelimiterRun = false, string end = null) : base(
            pattern: pattern) {
            this.endPattern = new Regex(end ?? pattern, options: RegexOptions.Multiline);
        ...
        internal override bool onMatch(InlineParser parser, Match match) {
            ...
            if (!this.requiresDelimiterRun) {
                parser.openTag(new TagState(startPos: parser.pos, matchEnd + 1, this, null));
                return true;
            }

            var delimiterRun = _DelimiterRun.tryParse(parser: parser, runStart: matchStart, runEnd: matchEnd);
            if (delimiterRun != null && delimiterRun.canOpen) {
                parser
                    .openTag(new TagState(startPos: parser.pos, matchEnd + 1, this, openingDelimiterRun: delimiterRun));
                return true;
            }
            else {
                parser.advanceBy(length: runLength);
                return false;
            }
        }
        ...
        internal virtual bool onMatchEnd(InlineParser parser, Match match, TagState state) {
            ...
            if (openingRunLength == 1 && runLength == 1) {
                parser.addNode(new Element("em", children: state.children));
            }
                ...
                parser.openTag(
                    new TagState(startPos: state.startPos, state.endPos - 1, this, openingDelimiterRun: delimiterRun));
                parser.addNode(new Element("em", children: state.children));
            }
            else if (openingRunLength == 2 && runLength == 2) {
                parser.addNode(new Element("strong", children: state.children));
            }
            else if (openingRunLength == 2 && runLength > 2) {
                parser.addNode(new Element("strong", children: state.children));
                ...
                parser.openTag(
                    new TagState(startPos: state.startPos, state.endPos - 2, this, openingDelimiterRun: delimiterRun));
                parser.addNode(new Element("strong", children: state.children));
            }
            else if (openingRunLength > 2 && runLength > 2) {
                parser.openTag(
    ...
    class StrikethroughSyntax : TagSyntax {
        ...
        internal override bool onMatchEnd(InlineParser parser, Match match, TagState state) {
            ...
            parser.addNode(new Element("del", children: state.children));
            return true;
        }
    }
    ...
    class LinkSyntax : TagSyntax {
        ...
        internal LinkSyntax(Resolver linkResolver = null, string pattern = @"\[") :
            base(pattern: pattern, false, @"\]") {
        ...
        internal override bool onMatch(InlineParser parser, Match match) {
            var matched = base.onMatch(parser: parser, match: match);
            if (!matched) {
                return false;
            }
        ...
        internal override bool onMatchEnd(InlineParser parser, Match match, TagState state) {
            ...
            var text = parser.source.substring(start: state.endPos, end: parser.pos);
            ...
            if (parser.pos + 1 >= parser.source.Length) {
                ...
                // reference link.
                return this._tryAddReferenceLink(parser: parser, state: state, label: text);
            }
...
// Maybe an inline link, like `[text](destination)`.
                ...
                var inlineLink = this._parseInlineLink(parser: parser);
                if (inlineLink != null) {
                    return this._tryAddInlineLink(parser: parser, state: state, link: inlineLink);
                }
...
// like `[foo][bar]` or a *collapsed* reference link, like `[foo][]`.
                ...
                var label = this._parseReferenceLinkLabel(parser: parser);
                if (label != null) {
                    return this._tryAddReferenceLink(parser: parser, state: state, label: label);
                }
...
// an opening `[`. Perhaps just a simple shortcut reference link (`[...]`).
        ...
        internal Node _resolveReferenceLink(
            ...
            if (linkReferences.ContainsKey(key: normalizedLabel)) {
                linkReference = linkReferences[key: normalizedLabel];
            }

            if (linkReference != null) {
                return this._createNode(state: state, destination: linkReference.destination,
...
// brackets, so that, e.g. a link resolver can receive `[\[\]]` as `[]`.
        ...
        protected virtual Node _createNode(TagState state, string destination, string title) {
            var element = new Element("a", children: state.children);
            element.attributes["href"] = Utils.escapeAttribute(value: destination);
            if (title != null && title.isNotEmpty()) {
                element.attributes["title"] = Utils.escapeAttribute(value: title);
            }
...
// Returns whether the link was added successfully.
        protected virtual bool _tryAddReferenceLink(InlineParser parser, TagState state, string label) {
            var element = this._resolveReferenceLink(label: label, state: state,
                linkReferences: parser.document.linkReferences);
            if (element == null) {
                return false;
            }

            parser.addNode(node: element);
            parser.start = parser.pos;
...
// Returns whether the link was added successfully.
        bool _tryAddInlineLink(InlineParser parser, TagState state, InlineLink link) {
            var element = this._createNode(state: state, destination: link.destination, title: link.title);
            if (element == null) {
                return false;
            }
...
// Walk past the opening `[`.
            ...
            while (true) {
                var cha = parser.charAt(index: parser.pos);
                if (cha == CharCode.backslash) {
                    parser.advanceBy(1);
                    var next = parser.charAt(index: parser.pos);
                    if (next != CharCode.backslash && next != CharCode.rbracket) {
                        buffer.writeCharCode(i: cha);
                    }

                    buffer.writeCharCode(i: next);
                }
...
// A link label must contain at least one non-whitespace character.
            if (_entirelyWhitespacePattern.hasMatch(content: label)) {
                return null;
            }
...
// Start walking to the character just after the opening `(`.
            parser.advanceBy(1);
            this._moveThroughWhitespace(parser: parser);
            if (parser.isDone) {
                return null; // EOF. Not a link.
            }

            if (parser.charAt(index: parser.pos) == CharCode.lt) {
// Maybe a `<...>`-enclosed link destination.
                return this._parseInlineBracketedLink(parser: parser);
            }
            else {
                return this._parseInlineBareDestinationLink(parser: parser);
            }
        }
        ...
        InlineLink _parseInlineBracketedLink(InlineParser parser) {
            ...
            var buffer = new StringBuilder();
            while (true) {
                cha = parser.charAt(index: parser.pos);
                if (cha == CharCode.backslash) {
                    parser.advanceBy(1);
                    var next = parser.charAt(index: parser.pos);
...
// Not a link (no whitespace allowed within `<...>`).
            ...
            if (cha == CharCode.space || cha == CharCode.lf || cha == CharCode.cr || cha == CharCode.ff) {
                var title = this._parseTitle(parser: parser);
                if (title == null && parser.charAt(index: parser.pos) != CharCode.rparen) {
// This looked like an inline link, until we found this $space
// followed by mystery characters; no longer a link.
                    return null;
                }

                return new InlineLink(destination: destination, title: title);
            }
            else if (cha == CharCode.rparen) {
                return new InlineLink(destination: destination);
            }
            else {
...
// http://spec.commonmark.org/0.28/#example-467
                    ...
                    case CharCode.ff:
                        ...
                        if (title == null && (parser.isDone || parser.charAt(index: parser.pos) != CharCode.rparen)) {
// This looked like an inline link, until we found this $space
// followed by mystery characters; no longer a link.
                            return null;
                        }
...
// destination.
        string _parseTitle(InlineParser parser) {
            ...
            var delimiter = parser.charAt(index: parser.pos);
...
// Advance past the closing delimiter.
            ...
            if (parser.charAt(index: parser.pos) != CharCode.rparen) {
                return null;
            }
    ...
    /// Matches videos like `!v[alternate text](url)`
    class VideoSyntax : LinkSyntax {
        public VideoSyntax(Resolver linkResolver = null)
            : base(linkResolver: linkResolver, @"!v\[") {
        }

        protected override Node _createNode(TagState state, string destination, string title) {
            var element = Element.empty("video");
            element.attributes["src"] = Utils.escapeHtml(html: destination);
            element.attributes["alt"] = state?.textContent ?? "";
            if (title != null && title.isNotEmpty()) {
                element.attributes["title"] = Utils.escapeAttribute(title.replaceAll("&", "&amp;"));
    ...
    class CodeSyntax : InlineSyntax {
        ...
        public CodeSyntax() : base(pattern: _pattern) {
        }
        ...
        internal override bool tryMatch(InlineParser parser, int startMatchPos) {
            ...
            var match = this.pattern.matchAsPrefix(content: parser.source, start: parser.pos);
            if (!match.Success) {
                return false;
            }

            parser.writeText();
            if (this.onMatch(parser: parser, match: match)) {
                parser.consume(length: match.Groups[0].Length);
            }
    ...
    class EmojiSyntax : InlineSyntax {
        ...
        internal override bool onMatch(InlineParser parser, Match match) {
            ...
            var emoji = Emojis.emojis[key: alias];
            if (emoji == null) {
                parser.advanceBy(1);
                return false;
            }

            parser.addNode(new Text(text: emoji));
    ...
    class TagState {
        ...
        internal bool tryMatch(InlineParser parser) {
            var endMatch = this.syntax.endPattern.matchAsPrefix(content: parser.source, start: parser.pos);
            if (!endMatch.Success) {
                return false;
            }
            ...
            if (!this.syntax.requiresDelimiterRun) {
                // Close the tag.
                this.close(parser: parser, endMatch: endMatch);
                return true;
            }
            ...
            var closingDelimiterRun =
                _DelimiterRun.tryParse(parser: parser, runStart: closingMatchStart, runEnd: closingMatchEnd);
        ...
        internal List<Node> close(InlineParser parser, Match endMatch) {
            ...
            parser._stack.removeRange(index + 1, count: parser._stack.Count);
            // Flatten them out onto this tag.
            foreach (var unmatched in unmatchedTags) {
                // Write the start tag as text.
                parser.writeTextRange(start: unmatched.startPos, end: unmatched.endPos);

                // Bequeath its children unto this tag.
                this.children.AddRange(collection: unmatched.children);
            }
            ...
            if (this.syntax.onMatchEnd(parser: parser, match: endMatch, this)) {
                parser.consume(length: endMatch.Groups[0].Length);
            }
            else {
                // Didn't close correctly so revert to text.
                parser.writeTextRange(start: this.startPos, end: this.endPos);
                parser._stack.LastOrDefault()?.children.AddRange(collection: this.children);
                parser.pos = endMatchIndex;
                parser.advanceBy(length: endMatch.Groups[0].Length);
            }
