...
namespace ConnectApp.Components.Markdown.html.htmlAgilityPack {
    ...
    public partial class HtmlDocument {
        ...
        /// </summary>
        public HtmlDocument() {
            this._documentnode = this.CreateNode(type: HtmlNodeType.Document, 0);
#if NETSTANDARD1_4
        ...
        public static string GetXmlName(string name) {
            ...
            for (var i = 0; i < name.Length; i++) {
                ...
                if (((name[index: i] >= 'a') && (name[index: i] <= 'z')) ||
                    ((name[index: i] >= '0') && (name[index: i] <= '9')) ||
                    //					(name[i]==':') || (name[i]=='_') || (name[i]=='-') || (name[i]=='.')) // these are bads in fact
                    (name[index: i] == '_') || (name[index: i] == '-') || (name[index: i] == '.')) {
                    xmlname += name[index: i];
                }
                else {
                    nameisok = false;
                    var bytes = Encoding.UTF8.GetBytes(new char[] {name[index: i]});
        ...
        static readonly Regex HtmlEncodePattern =
            new Regex("&(?!(amp;)|(lt;)|(gt;)|(quot;))", options: RegexOptions.IgnoreCase);
        ...
        public static string HtmlEncode(string html) {
            ...
            return HtmlEncodePattern.Replace(input: html, "&amp;").Replace("<", "&lt;").Replace(">", "&gt;")
                .Replace("\"", "&quot;");
        }
        ...
        public HtmlAttribute CreateAttribute(string name, string value) {
            ...
            var att = this.CreateAttribute(name: name);
            att.Value = value;
            return att;
        }
        ...
        public HtmlCommentNode CreateComment() {
            return (HtmlCommentNode) this.CreateNode(type: HtmlNodeType.Comment);
        }
        ...
        public HtmlNode CreateElement(string name) {
            ...
            var node = this.CreateNode(type: HtmlNodeType.Element);
            node.Name = name;
            return node;
        }
        ...
        public HtmlTextNode CreateTextNode() {
            return (HtmlTextNode) this.CreateNode(type: HtmlNodeType.Text);
        }
        ...
        public Encoding DetectEncoding(Stream stream) {
            ...
            return this.DetectEncoding(new StreamReader(stream: stream));
        }
        ...
        public Encoding DetectEncodingHtml(string html) {
            ...
            using (var sr = new StringReader(s: html)) {
                var encoding = this.DetectEncoding(reader: sr);
                return encoding;
            }
        }
        ...
        public HtmlNode GetElementbyId(string id) {
            ...
            if (this.Nodesid == null) {
                throw new Exception(message: HtmlExceptionUseIdAttributeFalse);
            }
        ...
        public void Load(Stream stream) {
            this.Load(new StreamReader(stream: stream, encoding: this.OptionDefaultStreamEncoding));
        }
        ...
        public void Load(Stream stream, bool detectEncodingFromByteOrderMarks) {
            this.Load(new StreamReader(stream: stream,
        ...
        public void Load(Stream stream, Encoding encoding) {
            this.Load(new StreamReader(stream: stream, encoding: encoding));
        }
        ...
        public void Load(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int buffersize) {
            this.Load(new StreamReader(stream: stream, encoding: encoding,
                detectEncodingFromByteOrderMarks: detectEncodingFromByteOrderMarks, bufferSize: buffersize));
        }
        ...
        public void Load(TextReader reader) {
            ...
            foreach (var node in this.Openednodes.Values) {
                ...
                if (this.OptionExtractErrorSourceText) {
                    ...
                    if (html.Length > this.OptionExtractErrorSourceTextMaxLength) {
                        html = html.Substring(0, length: this.OptionExtractErrorSourceTextMaxLength);
                    }
                }
                else {
                    html = string.Empty;
                ...
                this.AddError(
                    code: HtmlParseErrorCode.TagNotClosed,
                    line: node._line, linePosition: node._lineposition,
                    streamPosition: node._streamposition, sourceText: html,
        ...
        public void Save(Stream outStream) {
            var sw = new StreamWriter(stream: outStream, this.GetOutEncoding());
            this.Save(writer: sw);
        }
        ...
        public void Save(Stream outStream, Encoding encoding) {
            ...
            var sw = new StreamWriter(stream: outStream, encoding: encoding);
            this.Save(writer: sw);
        }
        ...
        public void Save(TextWriter writer) {
            ...
            this.DocumentNode.WriteTo(outText: writer);
            writer.Flush();
        }
        ...
        public void Save(XmlWriter writer) {
            this.DocumentNode.WriteTo(writer: writer);
            writer.Flush();
        }
        ...
        internal HtmlNode CreateNode(HtmlNodeType type) {
            return this.CreateNode(type: type, -1);
        }

        internal HtmlNode CreateNode(HtmlNodeType type, int index) {
            switch (type) {
                case HtmlNodeType.Comment:
                    return new HtmlCommentNode(this, index: index);

                case HtmlNodeType.Text:
                    return new HtmlTextNode(this, index: index);

                default:
                    return new HtmlNode(type: type, this, index: index);
            }
        }
        ...
        internal HtmlNode GetXmlDeclaration() {
            ...
            foreach (var node in this._documentnode._childnodes) {
        ...
        void AddError(HtmlParseErrorCode code, int line, int linePosition, int streamPosition, string sourceText,
            string reason) {
            var err = new HtmlParseError(code: code, line: line, linePosition: linePosition,
                streamPosition: streamPosition, sourceText: sourceText, reason: reason);
            this._parseerrors.Add(item: err);
            return;
        }
        ...
        void CloseCurrentNode() {
            ...
            var error = false;
            var prev = Utilities.GetDictionaryValueOrNull(dict: this.Lastnodes, key: this._currentnode.Name);

            // find last node of this kind
            if (prev == null) {
                if (HtmlNode.IsClosedElement(name: this._currentnode.Name)) {
                    // </br> will be seen as <br>
                    this._currentnode.CloseNode(endnode: this._currentnode);

                    // add to parent node
                    if (this._lastparentnode != null) {
                        HtmlNode foundNode = null;
                        var futureChild = new Stack<HtmlNode>();
                        for (var node = this._lastparentnode.LastChild;
                            node != null;
                            ...
                            futureChild.Push(item: node);
                        }

                        if (foundNode != null) {
                            while (futureChild.Count != 0) {
                                var node = futureChild.Pop();
                                this._lastparentnode.RemoveChild(oldChild: node);
                                foundNode.AppendChild(newChild: node);
                            }
                        }
                        else {
                            this._lastparentnode.AppendChild(newChild: this._currentnode);
                        }
                    }
                }
                else {
                    ...
                    if (HtmlNode.CanOverlapElement(name: this._currentnode.Name)) {
                        // this is a hack: add it as a text node
                        var closenode = this.CreateNode(type: HtmlNodeType.Text,
                            index: this._currentnode._outerstartindex);
                        closenode._outerlength = this._currentnode._outerlength;
                        ((HtmlTextNode) closenode).Text = ((HtmlTextNode) closenode).Text.ToLower();
                        if (this._lastparentnode != null) {
                            this._lastparentnode.AppendChild(newChild: closenode);
                        }
                    }
                    else {
                        if (HtmlNode.IsEmptyElement(name: this._currentnode.Name)) {
                            this.AddError(
                                code: HtmlParseErrorCode.EndTagNotRequired, line: this._currentnode._line,
                                linePosition: this._currentnode._lineposition,
                                streamPosition: this._currentnode._streamposition,
                                sourceText: this._currentnode.OuterHtml,
                                "End tag </" + this._currentnode.Name + "> is not required");
                        }
                        else {
                            // node cannot overlap, node is not empty
                            this.AddError(
                                code: HtmlParseErrorCode.TagNotOpened, line: this._currentnode._line,
                                linePosition: this._currentnode._lineposition,
                                streamPosition: this._currentnode._streamposition,
                                sourceText: this._currentnode.OuterHtml,
                ...
                if (this.OptionFixNestedTags) {
                    if (this.FindResetterNodes(node: prev, this.GetResetters(name: this._currentnode.Name))) {
                        this.AddError(
                            code: HtmlParseErrorCode.EndTagInvalidHere, line: this._currentnode._line,
                            linePosition: this._currentnode._lineposition,
                            streamPosition: this._currentnode._streamposition,
                            sourceText: this._currentnode.OuterHtml,
                ...
                if (!error) {
                    this.Lastnodes[key: this._currentnode.Name] = prev._prevwithsamename;
                    prev.CloseNode(endnode: this._currentnode);
                }
            }
            ...
            if (!error) {
                if ((this._lastparentnode != null) &&
                    ((!HtmlNode.IsClosedElement(name: this._currentnode.Name)) ||
        ...
        string CurrentNodeName() {
            return this.Text.Substring(startIndex: this._currentnode._namestartindex,
                length: this._currentnode._namelength);
        }
        ...
        HtmlNode FindResetterNode(HtmlNode node, string name) {
            var resetter = Utilities.GetDictionaryValueOrNull(dict: this.Lastnodes, key: name);
            if (resetter == null) {
                return null;
            }
        ...
        bool FindResetterNodes(HtmlNode node, string[] names) {
            ...
            for (var i = 0; i < names.Length; i++) {
                if (this.FindResetterNode(node: node, names[i]) != null) {
                    return true;
                }
            }
        ...
        void FixNestedTag(string name, string[] resetters) {
            ...
            if (prev == null || (this.Lastnodes[key: name].Closed)) {
                return;
            }

            // try to find a resetter node, if found, we do nothing
            if (this.FindResetterNodes(node: prev, names: resetters)) {
                return;
            }

            // ok we need to close the prev now
            // create a fake closer node
            var close = new HtmlNode(type: prev.NodeType, this, -1);
            close._endnode = close;
            prev.CloseNode(endnode: close);
        }
        ...
        void FixNestedTags() {
            ...
            this.FixNestedTag(name: name, this.GetResetters(name: name));
        }
        ...
        void IncrementPosition() {
            if (this._crc32 != null) {
                ...
                this._crc32.AddToCRC32(c: this._c);
            }
        ...
        bool NewCheck() {
            ...
            if (this._index < this.Text.Length) {
                if (this.Text[index: this._index] == '%') {
                    switch (this._state) {
            ...
            if ((this._index - 1) <= (this.Text.Length - 2)) {
                if (this.Text[index: this._index] == '!') {
                    this.PushNodeStart(type: HtmlNodeType.Comment, this._index - 1);
                    this.PushNodeNameStart(true, index: this._index);
            ...
            this.PushNodeStart(type: HtmlNodeType.Element, this._index - 1);
            return true;
        }
        ...
        void Parse() {
            ...
            this._lastparentnode = this._documentnode;
            this._currentnode = this.CreateNode(type: HtmlNodeType.Text, 0);
            this._currentattribute = null;

            this._index = 0;
            this.PushNodeStart(type: HtmlNodeType.Text, 0);
            while (this._index < this.Text.Length) {
                this._c = this.Text[index: this._index];
                ...
                switch (this._state) {
                    ...
                    case ParseState.WhichTag:
                        ...
                        if (this._c == '/') {
                            this.PushNodeNameStart(false, index: this._index);
                        }
                        else {
                            this.PushNodeNameStart(true, this._index - 1);
                    ...
                    case ParseState.Tag:
                        ...
                        if (this._c == '>') {
                            ...
                            if (!this.PushNodeEnd(index: this._index, false)) {
                                // stop parsing
                                this._index = this.Text.Length;
                                break;
                            }
                            ...
                            this._state = ParseState.Text;
                            this.PushNodeStart(type: HtmlNodeType.Text, index: this._index);
                        }
                    ...
                    case ParseState.EmptyTag:
                        ...
                        if (this._c == '>') {
                            if (!this.PushNodeEnd(index: this._index, true)) {
                                // stop parsing
                                this._index = this.Text.Length;
                                break;
                            }
                    ...
                    case ParseState.AttributeAfterEquals:
                        ...
                        if ((this._c == '\'') || (this._c == '"')) {
                            ...
                            this.PushAttributeValueStart(index: this._index, quote: this._c);
                            lastquote = this._c;
                            continue;
                        }
                        ...
                        if ((this._currentnode._namelength + 3) <= (this.Text.Length - (this._index - 1))) {
                            if (string.Compare(this.Text.Substring(this._index - 1, this._currentnode._namelength + 2),
                                ...
                                if ((c == '>') || (IsWhiteSpace(c: c))) {
                                    ...
                                    var script = this.CreateNode(type: HtmlNodeType.Text,
                                    ...
                                    this._currentnode.AppendChild(newChild: script);
            ...
            if (this._currentnode._namestartindex > 0) {
                this.PushNodeNameEnd(index: this._index);
            }

            this.PushNodeEnd(index: this._index, false);
        ...
        void PushAttributeNameEnd(int index) {
            ...
            this._currentnode.Attributes.Append(newAttribute: this._currentattribute);
        }
        ...
        void PushAttributeValueStart(int index) {
            this.PushAttributeValueStart(index: index, 0);
        }
        ...
        bool PushNodeEnd(int index, bool close) {
            ...
            if ((this._currentnode._nodetype == HtmlNodeType.Text) ||
                ...
                if ((this._currentnode._starttag) && (this._lastparentnode != this._currentnode)) {
                    ...
                    this.ReadDocumentEncoding(node: this._currentnode);

                    // remember last node of this kind
                    var prev = Utilities.GetDictionaryValueOrNull(dict: this.Lastnodes, key: this._currentnode.Name);

                    this._currentnode._prevwithsamename = prev;
                    this.Lastnodes[key: this._currentnode.Name] = this._currentnode;
                    ...
                    if ((HtmlNode.IsClosedElement(name: this._currentnode.Name)) ||
                        (HtmlNode.IsEmptyElement(name: this._currentnode.Name))) {
                        close = true;
                    }
                }
            }
            ...
            if ((close) || (!this._currentnode._starttag)) {
                if ((this.OptionStopperNodeName != null) && (this._remainder == null) &&
                    (string.Compare(strA: this._currentnode.Name, strB: this.OptionStopperNodeName,
                        comparisonType: StringComparison.OrdinalIgnoreCase) == 0)) {
                    this._remainderOffset = index;
                    this._remainder = this.Text.Substring(startIndex: this._remainderOffset);
        ...
        void PushNodeStart(HtmlNodeType type, int index) {
            this._currentnode = this.CreateNode(type: type, index: index);
        ...
        void ReadDocumentEncoding(HtmlNode node) {
            ...
            if (string.Compare(strA: att.Value, "content-type", comparisonType: StringComparison.OrdinalIgnoreCase) !=
                0) {
                return;
            }
            ...
            if (content != null) {
                var charset = NameValuePairList.GetNameValuePairsValue(text: content.Value, "charset");
                if (!string.IsNullOrEmpty(value: charset)) {
                    // The following check fixes the bug described at: http://htmlagilitypack.codeplex.com/WorkItem/View.aspx?WorkItemId=25273
                    if (string.Equals(a: charset, "utf8", comparisonType: StringComparison.OrdinalIgnoreCase)) {
                        charset = "utf-8";
                    }

                    try {
                        this._declaredencoding = Encoding.GetEncoding(name: charset);
                    }
                    ...
                    if (this._onlyDetectEncoding) {
                        throw new EncodingFoundException(encoding: this._declaredencoding);
                    }
                    ...
                    if (this._streamencoding != null) {
                        ...
                        if (this._declaredencoding != null) {
                            ...
                            {
                                this.AddError(
                                    code: HtmlParseErrorCode.CharsetMismatch, line: this._line,
                                    linePosition: this._lineposition, streamPosition: this._index,
                                    sourceText: node.OuterHtml,
