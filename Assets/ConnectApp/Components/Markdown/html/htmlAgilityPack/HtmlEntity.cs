    ...
    public class HtmlEntity {
                                ...
                                if (entity.Length == 0) {
                                    ...
                                    if (entity[0] == '#') {
                                        ...
                                        try {
                                            ...
                                            var code = Convert.ToInt32(value: codeStr, fromBase: fromBase);
                                            sb.Append(Convert.ToChar(value: code));
                                        }
                                        catch {
                                            sb.Append("&#" + e + ";");
                                        }
                                    ...
                                    entity.Remove(0, length: entity.Length);
                                }
        ...
        public static HtmlNode Entitize(HtmlNode node) {
            ...
            if (result.HasAttributes) {
                Entitize(collection: result.Attributes);
            }

            if (result.HasChildNodes) {
                Entitize(collection: result.ChildNodes);
            }
            else {
                if (result.NodeType == HtmlNodeType.Text) {
                    ((HtmlTextNode) result).Text = Entitize(text: ((HtmlTextNode) result).Text, true, true);
                }
            }
        ...
        public static string Entitize(string text) {
            return Entitize(text: text, true);
        }
        ...
        public static string Entitize(string text, bool useNames) {
            return Entitize(text: text, useNames: useNames, false);
        }
...
//		_entityName.Add(62, "gt");
        {
            ...
            var sb = new StringBuilder(capacity: text.Length);
            for (var i = 0; i < text.Length; i++) {
                int code = text[index: i];
                if ((code > 127) ||
                    (entitizeQuotAmpAndLtGt && ((code == 34) || (code == 38) || (code == 60) || (code == 62)))) {
                    var entity = _entityName[key: code] as string;
        ...
        static void Entitize(HtmlAttributeCollection collection) {
            foreach (var at in collection) {
                at.Value = Entitize(text: at.Value);
            }
        }
        ...
        static void Entitize(HtmlNodeCollection collection) {
            foreach (var node in collection) {
                if (node.HasAttributes) {
                    Entitize(collection: node.Attributes);
                }

                if (node.HasChildNodes) {
                    Entitize(collection: node.ChildNodes);
                }
                else {
                    if (node.NodeType == HtmlNodeType.Text) {
                        ((HtmlTextNode) node).Text = Entitize(text: ((HtmlTextNode) node).Text, true, true);
                    }
                }
            }
        }
