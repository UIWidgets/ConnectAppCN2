...
namespace ConnectApp.Components {
    ...
    class _PopupLikeButtonBarState : TickerProviderStateMixin<PopupLikeButtonBar> {
        ...
        public override void initState() {
            ...
            this._barSize = new FloatTween(0.7f, 1.0f).animate(parent: this._barSizeController);
            ...
            this._buttonSize = new FloatTween(0, 1.0f).animate(parent: this._buttonSizeController);
        ...
        public override Widget build(BuildContext context) {
            var popupLikeButtons = new List<Widget>();
            for (var i = 0; i < this.widget.popupLikeButtonData.Count; i++) {
                var popupLikeButtonItem = this.widget.popupLikeButtonData[index: i];
                popupLikeButtons.Add(this._buildPopupLikeButton(
                    content: popupLikeButtonItem.content,
                    selected: popupLikeButtonItem.selected,
                    i == 0,
                    i == this.widget.popupLikeButtonData.Count - 1,
                    () => { this.widget.onTap(type: popupLikeButtonItem.type); }
                ));
            }
            ...
            result = new FractionalTranslation(
                ...
                child: new Transform(
                    transform: Matrix4.translationValues(x: this._barSize.value, y: this._barSize.value,
                        z: this._barSize.value),
        ...
        Widget _buildPopupLikeButton(
            ...
            return new GestureDetector(
                onTap: onTap,
                child: new Container(
                    padding: EdgeInsets.only(leftMost ? 0 : 2, right: rightMost ? 0 : 2),
                    child: new Container(
                        width: 44,
                        height: 44,
                        decoration: new BoxDecoration(
