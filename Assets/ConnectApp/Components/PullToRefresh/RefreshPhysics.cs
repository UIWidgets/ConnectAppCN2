...
namespace ConnectApp.Components.PullToRefresh {
    public class RefreshScrollPhysics : ScrollPhysics {
        ...
        public override ScrollPhysics applyTo(ScrollPhysics ancestor) {
            return new RefreshScrollPhysics(this.buildParent(ancestor: ancestor),
        ...
        public override float applyPhysicsToUserOffset(ScrollMetrics position, float offset) {
            ...
            var overScrollPast = Mathf.Max(a: overScrollPastStart, b: overScrollPastEnd);
            var easing = (overScrollPastStart > 0.0 && offset < 0.0) ||
            ...
            return direction * _applyFriction(extentOutside: overScrollPast, offset.abs(), gamma: friction);
        }
