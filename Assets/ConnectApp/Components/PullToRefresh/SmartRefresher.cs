...
namespace ConnectApp.Components.PullToRefresh {
    ...
    public class SmartRefresher : StatefulWidget {
        public SmartRefresher(
            ...
            this.headerBuilder =
                headerBuilder ?? ((context, mode) => new SmartRefreshHeader(mode: mode, type: RefreshHeaderType.other));
            this.footerBuilder = footerBuilder ?? ((context, mode) => new SmartRefreshFooter(
                mode: mode,
                hasBottomMargin
                    ? EdgeInsets.only(0, 16, 0,
                        16 + CSizes.TabBarHeight +
    ...
    public class _SmartRefresherState : State<SmartRefresher> {
        ...
        bool _handleScrollStart(ScrollStartNotification notification) {
            ...
            var topWrap = this._headerKey.currentState as GestureProcessor;
            var bottomWrap = this._footerKey.currentState as GestureProcessor;
            if (this.widget.enablePullUp) {
                bottomWrap.onDragStart(notification: notification);
            }

            if (this.widget.enablePullDown) {
                topWrap.onDragStart(notification: notification);
            }
        ...
        bool _handleScrollMoving(ScrollUpdateNotification notification) {
            if (this._measure(notification: notification) != -1.0) {
                this.offsetLis.value = this._measure(notification: notification);
            }

            var topWrap = this._headerKey.currentState as GestureProcessor;
            var bottomWrap = this._footerKey.currentState as GestureProcessor;
            if (this.widget.enablePullUp) {
                bottomWrap.onDragMove(notification: notification);
            }

            if (this.widget.enablePullDown) {
                topWrap.onDragMove(notification: notification);
            }
        ...
        bool _handleScrollEnd(ScrollNotification notification) {
            ...
            var bottomWrap = this._footerKey.currentState as GestureProcessor;
            if (this.widget.enablePullUp) {
                bottomWrap.onDragEnd(notification: notification);
            }

            if (this.widget.enablePullDown) {
                topWrap.onDragEnd(notification: notification);
            }
        ...
        bool _dispatchScrollEvent(ScrollNotification notification) {
            this.widget.onNotification?.Invoke(notification: notification);
            ...
            if (!_isPullUp(notification: notification) && !_isPullDown(notification: notification)) {
                return false;
            }

            if (notification is ScrollStartNotification startNotification) {
                return this._handleScrollStart(notification: startNotification);
            }
            ...
            if (notification is ScrollUpdateNotification updateNotification) {
                ...
                if (updateNotification.dragDetails == null) {
                    return this._handleScrollEnd(notification: notification);
                }

                if (updateNotification.dragDetails != null) {
                    return this._handleScrollMoving(notification: updateNotification);
                }
            }

            if (notification is ScrollEndNotification) {
                this._handleScrollEnd(notification: notification);
            }
        ...
        void _init() {
            ...
            this._scrollController.addListener(listener: this._handleOffsetCallback);
            ...
            if (overScrollPastStart > overScrollPastEnd) {
                if (this.widget.headerConfig is RefreshConfig) {
                    if (this.widget.onOffsetChange != null) {
                        this.widget.onOffsetChange(true, offset: overScrollPastStart);
                    }
                }
                else {
                    if (this.widget.onOffsetChange != null) {
                ...
                if (this.widget.footerConfig is RefreshConfig) {
                    if (this.widget.onOffsetChange != null) {
                        this.widget.onOffsetChange(false, offset: overScrollPastEnd);
                    }
                }
                else {
                    if (this.widget.onOffsetChange != null) {
        ...
        void _onAfterBuild() {
            ...
            this.topModeLis.addListener(() => { this._didChangeMode(true, mode: this.topModeLis); });
            this.bottomModeLis.addListener(() => { this._didChangeMode(false, mode: this.bottomModeLis); });
            this.setState(() => {
                if (this.widget.enablePullDown && this._headerKey.currentContext != null) {
                    this._headerHeight = this._headerKey.currentContext.size.height;
                }

                if (this.widget.enablePullUp && this._footerKey.currentContext != null) {
        ...
        public override void dispose() {
            this._scrollController.removeListener(listener: this._handleOffsetCallback);
        ...
        public override Widget build(BuildContext context) {
            ...
            objs[0] = context;
            var slivers = (List<Widget>) method.Invoke(obj: this.widget.child, parameters: objs);
            slivers.Add(new SliverToBoxAdapter(
                child: this.widget.footerBuilder != null && this.widget.enablePullUp
                    ? this._buildWrapperByConfig(config: this.widget.footerConfig, false)
                    : new Container()
            ));
            slivers.Insert(
                0,
                new SliverToBoxAdapter(
                    child: this.widget.headerBuilder != null && this.widget.enablePullDown
                        ? this._buildWrapperByConfig(config: this.widget.headerConfig, true)
                        : new Container()));
    ...
    public class RefreshController {
        ...
        public void scrollTo(float offset) {
            this.scrollController?.jumpTo(value: offset);
        }

        public void animateTo(float to, TimeSpan duration, Curve curve) {
            this.scrollController?.animateTo(to: to, duration: duration, curve: curve);
        }
