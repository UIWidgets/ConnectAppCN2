...
namespace ConnectApp.Components.Swiper {
    public class WarmPainter : BasePainter {
        ...
        protected override void draw(Canvas canvas, float space, float size, float radius) {
            var progress = this.page - this.index;
            var distance = size + space;
            var start = this.index * (size + space);

            if (progress > 0.5f) {
                var right = start + size + distance;

                var left = this.index * distance + distance * (progress - 0.5f) * 2;
                canvas.drawRRect(
                    RRect.fromLTRBR(left: left, 0.0f, right: right, bottom: size, Radius.circular(radius: radius)),
                    paint: this._paint);
            }
            else {
                var right = start + size + distance * progress * 2;
    ...
    public class DropPainter : BasePainter {
        ...
        protected override void draw(Canvas canvas, float space, float size, float radius) {
            var progress = this.page - this.index;
            var dropHeight = this.widget.dropHeight;
            var rate = (0.5f - progress).abs() * 2;
            var scale = this.widget.scale;
    ...
    public class NonePainter : BasePainter {
        ...
        protected override void draw(Canvas canvas, float space, float size, float radius) {
            var progress = this.page - this.index;
            var secondOffset = this.index == this.widget.count - 1
                ? radius
    ...
    public class ScalePainter : BasePainter {
        ...
        protected override void draw(Canvas canvas, float space, float size, float radius) {
            ...
            canvas.drawCircle(new Offset(dx: secondOffset, dy: radius),
                this.lerp(radius * this.widget.scale, end: radius, progress: progress),
                paint: this._paint);
        }
    }
    ...
    public abstract class BasePainter : AbstractCustomPainter {
        ...
        public override bool shouldRepaint(CustomPainter _oldDelegate) {
            var oldDelegate = _oldDelegate as BasePainter;
    ...
    class _PageIndicatorState : State<PageIndicator> {
        ...
        void _onController() {
            var page = this.widget.controller.page;
        ...
        public override void didUpdateWidget(StatefulWidget _oldWidget) {
            var oldWidget = _oldWidget as PageIndicator;
