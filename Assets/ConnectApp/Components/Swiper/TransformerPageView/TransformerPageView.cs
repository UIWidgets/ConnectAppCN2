...
namespace ConnectApp.Components.Swiper {
    ...
    public class TransformerPageController : PageController {
        ...
        public int getRenderIndexFromRealIndex(int index) {
            return _getRenderIndexFromRealIndex(index: index, loop: this.loop, itemCount: this.itemCount,
        ...
        public override float page {
            get {
                return this.loop
                    ? _getRenderPageFromRealPage(page: this.realPage, loop: this.loop, itemCount: this.itemCount,
                        reverse: this.reverse)
                    : this.realPage;
            }
        }
        ...
        public int getRealIndexFromRenderIndex(int index) {
            return _getRealIndexFromRenderIndex(index: index, loop: this.loop, itemCount: this.itemCount,
                reverse: this.reverse);
        }

        static int _getRealIndexFromRenderIndex(
            int index, bool loop, int? itemCount, bool reverse) {
            var result = reverse ? (itemCount.Value - index - 1) : index;
            if (loop) {
    ...
    public class TransformerPageView : StatefulWidget {
        ...
        public TransformerPageView(
            ...
            this.duration =
                duration ?? TimeSpan.FromMilliseconds(value: TransformerPageViewUtils.kDefaultTransactionDuration);
            this.index = index;
    ...
    class _TransformerPageViewState : State<TransformerPageView> {
        ...
        Widget _buildItemNormal(BuildContext context, int index) {
            var renderIndex = this._pageController.getRenderIndexFromRealIndex(index: index);
            var child = this.widget.itemBuilder(context: context, index: renderIndex);
            return child;
        }
            ...
            return new AnimatedBuilder(
                ...
                (c, w) => {
                    ...
                    if (this.widget.itemBuilder != null) {
                        child = this.widget.itemBuilder(context: context, index: renderIndex);
                    }
                    ...
                    var page = this._pageController.realPage;
                    ...
                    var info = new TransformInfo(
                        ...
                        activeIndex: this._pageController.getRenderIndexFromRealIndex(index: this._activeIndex),
                    ...
                    return this._transformer.transform(child: child, info: info);
                });
        }

        float _calcCurrentPixels() {
            this._currentPixels = this._pageController.getRenderIndexFromRealIndex(index: this._activeIndex) *
        ...
        public override Widget build(BuildContext context) {
            var builder = this._transformer == null
        ...
        void _onIndexChanged(int index) {
            ...
            this.widget.onPageChanged?.Invoke(this._pageController.getRenderIndexFromRealIndex(index: index));
        }

        void _onGetSize(TimeSpan _) {
            Size size = null;
            if (this.context == null) {
                this.onGetSize(size: size);
                return;
            }
        ...
        public override void initState() {
            ...
            this._controller?.addListener(listener: this.onChangeNotifier);

            base.initState();
        }

        public override void didUpdateWidget(StatefulWidget _oldWidget) {
            var oldWidget = _oldWidget as TransformerPageView;
            this._transformer = this.widget.transformer;
            var index = this.widget.index ?? 0;
            var created = false;
            ...
            if (this._pageController.getRenderIndexFromRealIndex(index: this._activeIndex) != index) {
                this._fromIndex = this._activeIndex = this._pageController.initialPage;
                if (!created) {
                    var initPage = this._pageController.getRealIndexFromRenderIndex(index: index);
                    this._pageController.animateToPage(page: initPage,
                        duration: this.widget.duration.Value, curve: this.widget.curve);
                }
            }

            if (this._transformer != null) {
                WidgetsBinding.instance.addPostFrameCallback(callback: this._onGetSize);
            }

            if (this._controller != this.getNotifier()) {
                this._controller?.removeListener(listener: this.onChangeNotifier);
        ...
        void onChangeNotifier() {
            var evt = this.widget.controller.evt;
            int index;
            switch (evt) {
                case IndexController.MOVE: {
                    index = this._pageController
                        .getRealIndexFromRenderIndex(index: this.widget.controller.index);
                }
                    break;
            ...
            if (this.widget.controller.animation == true) {
                this._pageController
                    .animateToPage(
                        page: index,
                        duration: this.widget.duration.Value,
                        this.widget.curve ?? Curves.ease
                    ).whenComplete(action: this.widget.controller.complete);
            }
            else {
                this._pageController.jumpToPage(page: index);
