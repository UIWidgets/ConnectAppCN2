...
extern "C"{
    void InitPlayer(char* url,char* cookie,float left,float top,float width,float height,bool isPop,char* verifyType,int limitSeconds);
...
void InitPlayer(char* url,char* cookie,float left,float top,float width,float height,bool isPop,char* verifyType,int limitSeconds){
    ...
    NSString* verifyTypeStr = [NSString stringWithCString:verifyType encoding:NSUTF8StringEncoding];
    [avp initPlayerWithVideoUrl:videoUrl cookie:cookieStr left:left top:top width:width height:height isPop:isPop verifyType:verifyTypeStr limitSeconds:limitSeconds];
}
