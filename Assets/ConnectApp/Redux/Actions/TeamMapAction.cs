    ...
    public static partial class CActions {
        public static object fetchTeam(string teamId) {
            return new ThunkAction<AppState>((dispatcher, getState) => {
                return TeamApi.FetchTeam(teamId: teamId)
                    .then(data => {
                        if (!(data is FetchTeamResponse teamResponse)) {
                            return;
                        }
                        dispatcher.dispatch<Future>(fetchTeamArticle(teamId: teamResponse.team.id, 1));
        ...
        public static object fetchTeamArticle(string teamId, int pageNumber) {
            return new ThunkAction<AppState>((dispatcher, getState) => {
                return TeamApi.FetchTeamArticle(teamId: teamId, pageNumber: pageNumber)
                    .then(data => {
                        if (!(data is FetchTeamArticleResponse teamArticleResponse)) {
                            return;
                        }
        ...
        public static object fetchTeamFollower(string teamId, int offset) {
            return new ThunkAction<AppState>((dispatcher, getState) => {
                ...
                return TeamApi.FetchTeamFollower(teamId: teamId, offset: offset)
                    .then(data => {
                        if (!(data is FetchFollowerResponse teamFollowerResponse)) {
                            return;
                        }
        ...
        public static object fetchTeamMember(string teamId, int pageNumber) {
            return new ThunkAction<AppState>((dispatcher, getState) => {
                return TeamApi.FetchTeamMember(teamId: teamId, pageNumber: pageNumber)
                    .then(data => {
                        if (!(data is FetchTeamMemberResponse teamMemberResponse)) {
                            return;
                        }
        ...
        public static object fetchFollowTeam(string followTeamId) {
            return new ThunkAction<AppState>((dispatcher, getState) => {
                return TeamApi.FetchFollowTeam(teamId: followTeamId)
                    .then(data => {
        ...
        public static object fetchUnFollowTeam(string unFollowTeamId) {
            return new ThunkAction<AppState>((dispatcher, getState) => {
                return TeamApi.FetchUnFollowTeam(teamId: unFollowTeamId)
                    .then(data => {
                        if (!(data is bool success)) {
                            return;
                        }
