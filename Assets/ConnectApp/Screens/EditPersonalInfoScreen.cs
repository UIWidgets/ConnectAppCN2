                ...
                builder: (context1, viewModel, dispatcher) => {
                    var actionModel = new EditPersonalInfoScreenActionModel {
                        editPersonalInfo = (fullName, title, jobRoleId, placeId) =>
                            dispatcher.dispatch<Future>(CActions.editPersonalInfo(fullName: fullName, title: title,
                                jobRoleId: jobRoleId, placeId: placeId)),
                        updateAvatar = image => dispatcher.dispatch<Future>(CActions.updateAvatar(image: image)),
    ...
    class _EditPersonalInfoScreenState : State<EditPersonalInfoScreen> {
        ...
        void _editPersonalInfo() {
            ...
            this.widget.actionModel.editPersonalInfo(
                ...
                .then(_ => this.updateAvatar(true))
                .catchError(error => this.updateAvatar(false));
        }

        void updateAvatar(bool editSuccess) {
            if (!editSuccess) {
                CustomDialogUtils.hiddenCustomDialog(context: this.context);
                CustomDialogUtils.showToast(context: this.context, "提交失败", iconData: CIcons.sentiment_dissatisfied);
                return;
            }
            ...
            if (this._pickedImage.isEmpty()) {
                ...
                this.widget.actionModel.updateAvatar(arg: this._pickedImage)
                    .then(_ => {
                        CustomDialogUtils.hiddenCustomDialog(context: this.context);
                        Navigator.pop(context: this.context);
                    })
                    .catchError(error => {
                        CustomDialogUtils.hiddenCustomDialog(context: this.context);
                        CustomDialogUtils.showToast(context: this.context, "提交失败",
                            ...
                            new Positioned(
                                ...
                                child: new Container(
                                    ...
                                    child: new Container(
                                        ...
                                        child: new Icon(icon: CIcons.camera_alt, size: 20, color: CColors.White))
                                )
                            )
                        })
                )
        ...
        Widget _buildRoleItem() {
            ...
            var name = DictData.jobRoleDict.ContainsKey(key: jobRoleName)
                ? DictData.jobRoleDict[key: jobRoleName]
                : jobRoleName;
            return new Container(
                color: CColors.White,
                height: 60,
                    ...
                    children: new List<Widget> {
                        ...
                        new Expanded(
                            ...
                            child: new GestureDetector(
                                onTap: () =>
                                    Navigator.pushNamed(context: this.context, routeName: NavigatorRoutes.PersonalRole),
                                child: new Container(
                                    color: CColors.Transparent,
                                    child: new Stack(
                                        fit: StackFit.expand,
