...
- (void)initPlayerWithVideoUrl:(NSString*)videoUrl cookie:(NSString*)cookie left:(CGFloat)left top:(CGFloat)top width:(CGFloat)width height:(CGFloat)height isPop:(BOOL)isPop verifyType:(NSString *)verifyType limitSeconds:(int)limitSeconds{
    _marginTop = top;
    if (videoUrl.length>0) {
        self.wmPlayer.playerModel = [self setupModelWithUrl:videoUrl cookie:cookie verifyType:verifyType limitSeconds:limitSeconds];
    }
...
-(WMPlayerModel *)setupModelWithUrl:(NSString *)videoUrl cookie:(NSString *)cookie verifyType:(NSString *)verifyType limitSeconds:(int)limitSeconds{
    ...
    model.verifyType = verifyType;
    model.limitSeconds = limitSeconds;
    return model;
}
- (void)wmplayer:(WMPlayer *)wmplayer clickedBuyLicenseButton:(UIButton *)shareBtn{
    UIWidgetsMethodMessage(@"player", @"BuyLicense", @[@""]);
}
- (void)wmplayer:(WMPlayer *)wmplayer clickedUpdateLicenseButton:(UIButton *)updateBtn{
    UIWidgetsMethodMessage(@"player", @"UpdateLicense", @[@""]);
}
...
- (void)configPlayerWithVideUrl:(NSString *)url cookie:(NSString *)cookie{
    if (self.wmPlayer) {
        [self.wmPlayer setPlayerModel:[self setupModelWithUrl:url cookie:cookie verifyType:@"empty" limitSeconds:0]];
    }
}
