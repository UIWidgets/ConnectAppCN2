...
static NSString *VerifyType_Empty = @"empty";
static NSString *VerifyType_License = @"license";
static NSString *VerifyType_Whitelist = @"whitelist";
...
-(void)initWMPlayer{
    ...
    self.warningView = [[UIView alloc] init];
    self.warningView.hidden = YES;
    UILabel *warningLabel = [[UILabel alloc] init];
    [warningLabel setText:@"暂无权限继续观看该视频"];
    [warningLabel setTextColor:[UIColor whiteColor]];
    [warningLabel setFont:[UIFont systemFontOfSize:16]];
    [self.warningView addSubview:warningLabel];
    [warningLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.mas_equalTo(self.warningView);
    }];
...
- (void)handleSingleTap:(UITapGestureRecognizer *)sender{
    if (self.updateView.hidden == NO || self.warningView.hidden == NO) {
        return;
    }
...
- (void)moviePlayDidEnd:(NSNotification *)notification {
    [self unfocusSession];
    if (![self.playerModel.verifyType isEqualToString:VerifyType_Empty]) {
        if ([VerifyType_License isEqualToString:self.playerModel.verifyType]) {
            self.updateView.hidden = NO;
        } else if ([VerifyType_Whitelist isEqualToString:self.playerModel.verifyType]) {
...
- (void)updateProgress:(UISlider *)slider{
    if (slider.value >= 0
        && ![self.playerModel.verifyType isEqualToString:VerifyType_Empty]
        && slider.value * [self duration] >= self.playerModel.limitSeconds) {
        [self showBlockView:self.playerModel.verifyType];
        return;
    }
...
- (void)syncScrubber {
    ...
    if (![self.playerModel.verifyType isEqualToString:VerifyType_Empty]
        && self.playerModel.limitSeconds >= 0) {
...
-(float)moveProgressControllWithTempPoint:(CGPoint)tempPoint{
    ...
    if (![self.playerModel.verifyType isEqualToString: VerifyType_Empty]
        && tempValue >= self.playerModel.limitSeconds) {
        [self showBlockView:self.playerModel.verifyType];
    }
...
- (void)showBlockView:(NSString *)verifyType {
    if ([VerifyType_License isEqualToString:verifyType]) {
        self.updateView.hidden = NO;
    } else if ([VerifyType_Whitelist isEqualToString:verifyType]) {
        self.warningView.hidden = NO;
    }
    [self resetWMPlayer];
    [self hiddenControlView];
